...
const ORION_PANEL_HTML = `<div class="orion-panel" id="orionPanel" hidden>
  ...
  <div class="orion-inner">
    <div class="orion-head" onmousedown="startMove(event)">
      <div class="row">
        <div class="left"><svg class="rmx-icon avatar-logo" ondblclick="skipToCategory(1)" title="Orion Assistant"><use href="#orion"></use></svg><span class="name" id="orionHeadTitle">Orion Assistant</span></div>
        <div class="icons">
...
const HELP_PROMPTS = {
  h1: { cat:0, icon:'trending_up', label:'How do I raise rents?',
    ...
    rich:`
      <p>Use <strong>Set Renewal Rent Increase</strong> to adjust rent for tenants whose leases are expiring.</p>
      <ol>
        <li><strong>Open the page:</strong> Rental Info → Leasing → Renewal Increases.</li>
        <li><strong>Filter:</strong> Pick your properties and a lease expiration date range to find who needs an increase.</li>
        <li><strong>Click Set Rent Increases:</strong> Choose how to raise it — by amount or percentage, to a new flat amount, to market rent, by a scripted calculation, or not at all.</li>
        <li><strong>Pick the charge:</strong> Choose which recurring charge to increase, and round the result if you want.</li>
        <li><strong>Review and save:</strong> Check the affected tenants and their new amounts, then save.</li>
      </ol>
      <p><strong>Note:</strong> Needs the "Set Renewal Rent Increases" privilege.</p>` },
  h2: { cat:0, icon:'assignment_turned_in', label:'How do I send renewal offers?',
    ...
    rich:`
      <p>Two workflows, depending on whether the offer has gone out yet.</p>
      <h4>Creating offers</h4>
      <ol>
        <li><strong>Open the page:</strong> Rental Info → Leasing → Create Renewal Offers.</li>
        <li><strong>Select tenants:</strong> Filter by property, expiration date, term, or unit type, then pick who to offer.</li>
        <li><strong>Set the terms:</strong> Let tenants keep their current term, take a new one, or choose between several — and adjust any recurring or one-time charges.</li>
        <li><strong>Review and send.</strong></li>
      </ol>
      <h4>Resending</h4>
      <p>On the Lease Renewal Board, click Resend Offer on a tenant's card for one, or in the column heading for all of them. You can change the expiration date and pick signers first.</p>
      <p><strong>Note:</strong> Residential leases only, and resending needs the status to be "Offer Sent" with a Blue Moon lease or a signable document on Tenant Web Access.</p>` },
  h3: { cat:0, icon:'payments', label:'How do I pay bills?',
    ...
    rich:`
      <p>Bills to vendors and owners are paid from the Pay Bills page.</p>
      <ol>
        <li><strong>Open the page:</strong> Payables → Bills → Pay Bills.</li>
        <li><strong>Narrow the list:</strong> Filter by property, vendor, bill or due date, invoice number, or job.</li>
        <li><strong>Select and pay:</strong> Tick the bills, then choose a payment method per bill in the Bank column — a bank or credit card, VendorPay, or Zego ePay. You can set one default for the whole page.</li>
        <li><strong>Finish:</strong> Follow the prompts. Some methods post straight away, others need approval.</li>
      </ol>
      <p><strong>Note:</strong> Needs the "Allow user to pay bills" privilege, and "Take owner Payments" as well if the bill is for the management company.</p>` },
  h4: { cat:0, icon:'summarize', label:'What report will tell me who is behind on rent?',
    ...
    rich:`
      <p>The <strong>Delinquency report</strong>. It lists tenants and prospects with delinquent charges as of a date you choose, with balances by property and a grand total.</p>
      <ol>
        <li><strong>Open the report:</strong> Receivables → Charges → Delinquency.</li>
        <li><strong>Choose properties</strong> and an "As of Date".</li>
        <li><strong>Filter by age:</strong> Optionally exclude tenants under a set number of delinquent days.</li>
        <li><strong>Pick Detail or Summary</strong> — every charge, or one total per tenant — then run it.</li>
      </ol>
      <p><strong>Important:</strong> It reads unpaid charges, not ending balances. A payment received but not yet applied still shows as delinquent.</p>` },
  h5: { cat:0, icon:'receipt_long', label:'Is there an easy way to enter expense receipts into Rent Manager?',
    ...
    rich:`
      <p>Yes — <strong>Smart Receipts</strong>.</p>
      <ol>
        <li><strong>Capture:</strong> Photograph a receipt with the Capture tool in rmAppSuite Pro, or upload a PDF or image.</li>
        <li><strong>AI extraction:</strong> Orion reads the amount, vendor and date off the image into a draft transaction.</li>
        <li><strong>Review:</strong> Check the details, add a memo, and classify it as a company card charge or a personal reimbursement.</li>
        <li><strong>Post it</strong> as a credit card transaction or as a bill to reimburse the employee.</li>
      </ol>
      <p>A dashboard tile shows receipts uploaded but not yet posted.</p>
      <p><strong>Note:</strong> Smart Receipts is a licensed feature, purchased separately.</p>` },
};
...
function scrollAnchorIntoView(body, idx, jump, mode){
  ...
  /* Height of everything from the anchor's own top down to the very end
     ...
     difference. */
  const tailHeight = contentBottomOffset(body, spacer) - rowContentTop;
  const needed = Math.max(0, body.clientHeight - tailHeight);

  /* If the anchor is already sitting fully inside the panel, there is
     nowhere worth going. Scrolling it to the very top would mean opening a
     spacer's worth of blank room, sliding the conversation up into it, and
     then — once the answer turns out to fit after all — sliding it back
     down when settleSpacer hands the room back. A short answer spent its
     whole life doing that round trip, and the two moves cancelled out.

     So: leave the view alone and let settleSpacer decide. It runs once the
     answer has stopped growing, when its real height is finally known, and
     positions from there. An answer that fits never moves the panel at all;
     one that outgrows it gets a single scroll at the end instead of one at
     each end. */
  const anchorAlreadyVisible = jump
  ...
  /* Between one message and the next the spacer may only GROW. This is what
     keeps a reply still while it builds. Every render recomputes `needed`
     against whatever of the answer has appeared so far, so it shrinks as the
     answer fills in — and each shrink narrows the scroll range under a view
     that is already scrolled near the end of it, so the browser clamps
     scrollTop and the whole conversation lurches upward. That was the
     juddering during a reply: not the reply moving, the floor moving.

     Holding the high-water mark until the answer is finished means nothing
     moves at all — the reply simply grows into room that is already there.
     settleSpacer hands the leftover back in one smooth motion at the end. */
  if (spacer){
    spacer.style.height = (jump ? needed : Math.max(currentSpacerHeight, needed)) + 'px';
  }

  if (jump){
    const bodyRect2 = body.getBoundingClientRect();
    const rowRect2 = row.getBoundingClientRect();
    const target = Math.round(body.scrollTop + (rowRect2.top - bodyRect2.top));
...
/* How tall the conversation actually is, ignoring the spacer.
   ...
   clientHeight, so while an answer is still shorter than the panel it reads
   as a full panel of content and overstates the real height. Everything
   downstream then under-sizes the spacer, the body never becomes scrollable
   enough to lift the reply to the top, and the pin silently clamps to zero —
   which is exactly why the reply used to come to rest partway down. Measure
   to the top of the spacer instead, which is where the content genuinely
   ends. */
function contentBottomOffset(body, spacer){
  if (!spacer) return body.scrollHeight;
  const pad = parseFloat(getComputedStyle(body).paddingBottom) || 0;
  return body.scrollTop + (spacer.getBoundingClientRect().top - body.getBoundingClientRect().top) + pad;
}

/* One way of moving the panel, so every scroll in a conversation reads the
   same. Jumps straight there when the distance is trivial (a jump that short
   is not perceived as motion, and animating it only delays what follows) or
   when the reader has asked for reduced motion.

   Marks the panel as scrolling under its own power for the duration. A
   smooth scroll emits the same scroll events a finger does, and without
   this the settle's "has the reader taken over?" check reads the panel's own
   animation as the reader grabbing it, and abandons the reply half-placed. */
let programmaticScrollUntil = 0;
function orionScrollIsProgrammatic(){ return Date.now() < programmaticScrollUntil; }

/* One animator that re-aims, instead of a new scroll each time.

   An answer arrives in pieces a few hundred milliseconds apart, and each
   piece wants the view a little further down. Calling the browser's own
   smooth scroll for each of them starts a fresh animation over the top of
   the one still running: the motion restarts from zero velocity every
   time, which is the stutter. Native smooth scrolling also has no way to
   change its destination mid-flight.

   So the target is a variable and the motion is continuous. A new
   destination while it is moving just moves the destination — the easing
   carries straight on from wherever it is, at whatever speed it had. The
   step is proportional to the distance left, so it accelerates away and
   decelerates in, and it is driven on a timer rather than a frame callback
   because a throttled callback would strand the panel mid-scroll. */
const SCROLL_EASE = 0.14;   /* fraction of the remaining distance per tick */
/* Ceiling on a single tick. Proportional easing alone starts at its fastest,
   so a long jump opens with one big lurch before settling — capping the step
   makes it ease in as well as out. ~16px a tick is around 1000px a second. */
const SCROLL_MAX_STEP = 16;
const scrollAnims = new WeakMap();

function orionCancelScroll(body){
  const a = scrollAnims.get(body);
  if (a){ clearInterval(a.timer); scrollAnims.delete(body); }
}

function smoothScrollTo(body, target){
  const clamp = v => Math.max(0, Math.min(v, body.scrollHeight - body.clientHeight));
  const reduced = matchMedia('(prefers-reduced-motion: reduce)').matches;

  if (reduced || Math.abs(clamp(target) - body.scrollTop) < 2){
    orionCancelScroll(body);
    programmaticScrollUntil = Date.now() + 200;
    body.scrollTop = clamp(target);
    return;
  }

  const running = scrollAnims.get(body);
  if (running){ running.target = target; return; }   /* re-aim, keep moving */

  const anim = { target };
  anim.timer = setInterval(() => {
    const to = clamp(anim.target);
    ...
    /* At least a pixel a tick, or a tiny remainder crawls forever; at most
       SCROLL_MAX_STEP, or a long jump lurches on its first frame. */
    const eased = gap * SCROLL_EASE;
    const size = Math.min(Math.max(Math.abs(eased), 1), SCROLL_MAX_STEP);
    body.scrollTop = from + Math.sign(gap) * size;
  }, 16);
  scrollAnims.set(body, anim);
}
...
function settleSpacer(body, anchorRow){
  ...
  /* If the reader scrolls while the answer is still arriving, the panel
     stops MOVING the view — whatever they scrolled to is where it stays.
     It does not stop tidying up: the room held open for the answer is still
     handed back once the answer stops growing, because abandoning that too
     stranded a panel's worth of emptiness under the conversation that the
     reader then had to scroll through. Only a scroll the panel did not
     perform itself counts, so `expected` is refreshed after every move it
     makes. */
  let expected = Math.round(body.scrollTop);
  let userScrolled = false;
  const onScroll = () => {
    if (orionScrollIsProgrammatic()){ expected = Math.round(body.scrollTop); return; }
  ...
  /* Scroll events say the view moved; they do not say who moved it, and a
     reader reaching for the wheel while the panel is mid-animation is
     indistinguishable from the animation itself by timing alone — which is
     exactly when someone is most likely to grab it. Input events are not
     ambiguous: a wheel, a drag, a page-down key only ever come from a
     person. Those end the panel's control immediately. */
  const onInput = () => {
    userScrolled = true;
    const h = settleTimers.get(body);
    if (h) h.userScrolled = true;
    orionCancelScroll(body);   /* let go of the view mid-glide, not after it */
  };
  const SCROLL_KEYS = ['PageUp','PageDown','Home','End','ArrowUp','ArrowDown',' '];
  const onKey = e => { if (SCROLL_KEYS.includes(e.key)) onInput(); };

  body.addEventListener('scroll', onScroll, { passive: true });
  body.addEventListener('wheel', onInput, { passive: true });
  body.addEventListener('touchmove', onInput, { passive: true });
  body.addEventListener('keydown', onKey);
  const stop = () => {
    clearInterval(timer);
    body.removeEventListener('scroll', onScroll);
    body.removeEventListener('wheel', onInput);
    body.removeEventListener('touchmove', onInput);
    body.removeEventListener('keydown', onKey);
  };

  const timer = setInterval(() => {
    if (!userScrolled) expected = Math.round(body.scrollTop);
    /* Look the spacer up every poll rather than holding a reference to it:
       ...
       every pass. */
    const spacer = body.querySelector('.scroll-spacer');
    if (!spacer || !anchorRow.isConnected){ stop(); return; }
    if (++polls > 100){ releaseSpacer(body); stop(); return; }

    const spacerH = spacer.offsetHeight;
    const contentH = contentBottomOffset(body, spacer);
    /* An answer still arriving carries data-revealing until its last block
       is placed and its last character typed. Height alone is not enough:
       the pause between the opening paragraph and the first block is
       seconds of perfect stillness in the middle of the answer. */
    if (body.querySelector('[data-revealing]') || contentH !== lastContentH){
    ...
    /* Reader took over: give the room back where they are, and leave the
       view exactly as they left it. */
    if (userScrolled){ releaseSpacer(body); return; }
...
function render(){
  ...
  /* The body keeps its own flex:1/min-height:0/overflow:auto in every mode.
     It used to be pinned to 'flex: 0 0 auto' while the sheet was open on
     the home screen, back when the sheet was a floating sibling that sized
     itself against the body. The sheet now sits inside the card, inside the
     body — so pinning the height only stopped the body scrolling and clipped
     the bottom of the sheet against the panel. */
  body.style.flex = '';
...
function revealMessage(idx){
  ...
  /* An answer is "still arriving" until the last block has been placed and
     the last character typed — which is not the same as "has not changed
     height for a moment". The pause between the opening paragraph finishing
     and the first block appearing is seconds of complete stillness,
     and settleSpacer used to read that as the end of the answer and place
     the view against half of it. The flag below is the honest signal; the
     settle waits for it to clear. */
  row.dataset.revealing = '1';
  let pending = 1;                      /* the reveal schedule itself */
  const done = () => { if (--pending <= 0) delete row.dataset.revealing; };
  const alsoWaitFor = () => { pending++; };
  ...
  let i = 0;
  const next = () => {
    if (!content.isConnected){ done(); return; }
    const el = blocks[i++];
    if (!el){ done(); return; }
    el.hidden = false;
    /* Prose fades slower than data — the lead paragraph is marked in the
       markup, a rich answer is marked here. See .rv-slow. */
    if (el.querySelector('.mrich')) el.classList.add('rv-slow');
    /* Unhiding and setting the end state together gives the browser no start
       to transition from, so it lands on opacity 1 with no fade. Reading a
       layout property in between forces the opacity-0 start to be computed,
       and the class that follows then animates from it.

       Deliberately not requestAnimationFrame: a throttled frame callback —
       a background tab, a hidden pane — leaves the answer at opacity 0
  ...
  /* A beat after the thinking indicator clears, so the answer arrives rather
     than replacing it in the same frame. */
  setTimeout(next, 260);
}

/* A formatted answer arrives a paragraph at a time, the way a person reads
   one — not a cursor crawling through a skeleton.

   Typing it character by character meant the whole shape landed first: the
   numbers 1 to 8, the bullets, the headings, all sitting there empty while
   text trickled in behind them. You could read the structure of an answer
   before a word of it existed, which is not how anything writes.

   So the unit is the paragraph, the heading, the list item. Each is hidden
   outright — display:none, so a list has no empty rows and its numbering
   counts only what has actually appeared — and they fade in one after
   another in reading order. Nothing is ever on screen unfinished.
...
/* Demoes the composer with a scripted walk through the prompt suggestions —
   ...
   The order is the sheet's own: every prompt in every category, read
   category by category exactly as Browse Prompt Suggestions lists them, so
   it opens on "How do I raise rents?" and the walk never drifts from what
   the sheet shows. Derived from PROMPTS rather than hand-listed, so adding
   ...
   reset by newChat(), so starting a new chat mid-sequence picks up where it
   left off rather than restarting at the first prompt. */
/* Prompts the walk skips. They stay in the sheet — someone browsing
   Prompt Suggestions still sees every question — but a demo that opens with
   three how-do-I answers in a row spends its first minutes on documentation
   before reaching anything Orion actually does with your data. */
const AUTOFILL_SKIP = ['h1', 'h2', 'h3'];

const HOME_AUTOFILL_SEQUENCE = CATS
  .map((_, cat) => Object.keys(PROMPTS).filter(k => PROMPTS[k].cat === cat))
  .flat()
  .filter(k => !AUTOFILL_SKIP.includes(k));
let homeAutoFillIndex = 0;

/* A way out of the Find Answers questions mid-demo, for when the room has
   seen enough of them and you want to be on the data ones. Double-click the
   Orion mark in the panel header: the walk jumps to the first prompt of the
   next category and carries on from there.

   Deliberately unmarked — it is a presenter's shortcut, not a feature, and a
   visible control for it would be one more thing to explain on a screen
   whose whole point is being uncluttered. A double-click on the logo is
   something nobody finds by accident. It confirms with a toast so you know
   it took, since there is nothing else on screen to tell you. */
function skipToCategory(cat){
  const first = HOME_AUTOFILL_SEQUENCE.findIndex(k => PROMPTS[k].cat === cat);
  if (first === -1) return;
  homeAutoFillIndex = first;
  /* Clear both, not whichever happens to be on screen — the other one keeps
     its half-typed question and refuses to autofill when you get back to it. */
  for (const id of ['homeInput', 'draftInput']){
    const el = document.getElementById(id);
    if (el){ el.value = ''; resetAsk(id); }
  }
  if (typeof RMX !== 'undefined' && RMX.toast) RMX.toast('Skipped to ' + CATS[cat].label, 'neutral');
}
function demoComposerAutoFill(elId){
  ...
  const typeNext = () => {
    ...
    if (i < text.length) setTimeout(typeNext, 34 + Math.random() * 46);
  };
  typeNext();
}
...
function renderMessage(m, idx, animate){
  ...
  let inner = lead
    ? (animate ? `<div class="text rv rv-slow" hidden>${esc(m.text)}</div>`
...
/* Open onto the newest response, not the oldest. Nothing scrolls the body on
   open — the anchor logic only moves it when a message is actually added — so
   a conversation restored from sessionStorage came back sitting at the very
   top of its own history, with the answer the user last read a screen or two
   below the fold. Runs on the frame after layout so the restored messages
   have been measured. */
