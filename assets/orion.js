...
function settleSpacer(body, anchorRow){
  ...
  /* If the reader scrolls while the answer is still arriving, they are
     reading rather than following, and the panel stops managing the view —
     no final reposition, no hand-back. Whatever they scrolled to is where it
     stays. Only a scroll the panel did not perform itself counts, so
     `expected` is refreshed after every move it makes. */
  ...
  const timer = setInterval(() => {
    ...
    expected = Math.round(body.scrollTop);
    /* Look the spacer up every poll rather than holding a reference to it:
       ...
       every pass. */
    const spacer = body.querySelector('.scroll-spacer');
    if (!spacer || !anchorRow.isConnected || ++polls > 60){ stop(); return; }
