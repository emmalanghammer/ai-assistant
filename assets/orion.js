...
/* How long the opening paragraph takes to type, whatever its length — capped
   rather than per-character so a long answer still finishes in a couple of
   seconds instead of crawling. Named because revealMessage waits
...
function revealMessage(idx){
  ...
  /* An answer is "still arriving" until the last block has been placed and
     ...
     and the first block appearing is TYPE_MS + 200 of complete stillness,
  ...
  const next = () => {
    ...
    if (!typedRich && el.querySelector('.mrich')){ typedRich = true; alsoWaitFor(); typeOutContent(el, done); }
