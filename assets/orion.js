...
function scrollAnchorIntoView(body, idx, jump, mode){
  ...
  /* An answer never moves the view. It arrives below whatever you are
     reading and stays there until you go to it — the chevron appears when
     there is more below the fold, and that is the only invitation.

     This used to pin the reply's first line to the top of the panel, which
     meant holding a spacer's worth of blank room open beneath the
     conversation so there was somewhere to scroll to. Both are gone: no
     room is reserved, nothing is repositioned. The spacer element stays at
     zero so the scroll maths has something to measure the end of the
     content against.

     Landing positions are different and still apply — opening the panel,
     or a chat from History, goes to the end of it. Those are arrivals, not
     a reply shifting under you mid-read. */
  if (spacer) spacer.style.height = '0px';
  if (jump) settleSpacer(body, row);
}
...
function settleSpacer(body, anchorRow){
  ...
  if (prev) clearInterval(prev.poll);
  ...
  const timer = setInterval(() => {
    ...
    /* The answer has stopped growing. There is nothing to position — the
       panel does not move for a reply any more — so this only hands back
       the spacer and refreshes the chevron, which is the thing that tells
       you there is more below the fold. */
    releaseSpacer(body);
  }, 200);
  const handles = { poll: timer, userScrolled: false };
